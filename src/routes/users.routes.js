...
const { requireAuth, requirePerfil, requireClienteAtivo } = require('../auth/middleware');
...
module.exports = function (db) {
  ...
  router.get('/', requireAuth, requirePerfil('admin', 'master'), async (req, res) => {
    try {
      ...
      const usuarios = await db.getUsuarios(clienteId);
  ...
  router.post('/', requireAuth, requirePerfil('admin', 'master'), requireClienteAtivo, async (req, res) => {
    try {
      ...
      const usuario = await db.createUsuario({ nome, email: email.toLowerCase(), senha_hash: hash, perfil: perfil || 'admin', permissoes: permissoes || {}, cliente_id: clienteId });
