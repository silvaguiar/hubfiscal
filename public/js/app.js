        ...
        const interval = setInterval(async () => {
          try {
            ...
            if (logText && logText !== 'Aguardando início do processo...') {
              const lines = logText.split('\n').filter(l => l.trim() !== '');
              log.innerHTML = lines.slice(-30).map(l => {
                if (l.includes('[debug]')) return `<div style="color:var(--text-muted);font-size:11px">${l}</div>`;
                if (l.includes('❌')) return `<div style="color:#f87171">${l}</div>`;
                return `<div>${l}</div>`;
              }).join('');
